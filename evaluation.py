...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    pbar = tqdm(enumerate(data_loader), total=len(data_loader), desc="Encoding data")

    for i, (images, captions, lengths, ids) in pbar:
        ...
        if img_embs is None:
            ...
            token = [0] * len(data_loader.dataset)
        # cache embeddings
        ...
        # Update progress bar description with additional info if needed
        pbar.set_postfix({'batch': i + 1, 'total': len(data_loader)})
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        start = time.time()
        sims = shard_attn_scores(model, img_embs, cap_embs, cap_lens, opt, shard_size=32)
        end = time.time()
        ...
        for i in range(5):
            ...
            start = time.time()
            sims = shard_attn_scores(model, img_embs_shard, cap_embs_shard, cap_lens_shard, opt,
                                     shard_size=32)
            end = time.time()
            print("calculate similarity time:", end - start)
...
def evalrank_ensemble(model_path1, model_path2, data_path=None, split='dev', fold5=False):
    ...
    print('Images: %d, Captions: %d' %
          (img_embs2.shape[0] / 5, cap_embs2.shape[0]))
    ...
    if not fold5:
        ...
        start = time.time()
        sims = shard_attn_scores(model, img_embs, cap_embs, cap_lens,opt, shard_size=32)
        sims2 = shard_attn_scores(model2, img_embs2, cap_embs2, cap_lens2, opt, shard_size=32)
        end = time.time()
        print("calculate similarity time:", end - start)
        sims = (sims + sims2) / 2
        ...
        for i in range(5):
            ...
            sims = shard_attn_scores(model, img_embs_shard, cap_embs_shard, cap_lens_shard, opt,
                                     shard_size=32)
            sims2 = shard_attn_scores(model2, img_embs_shard2, cap_embs_shard2, cap_lens_shard2, opt,
                                      shard_size=32)
            end = time.time()
            print("calculate similarity time:", end - start)
            sims = (sims + sims2) / 2
...
def shard_attn_scores(model, img_embs, cap_embs, cap_lens, opt, shard_size=32):
    ...
    # 使用单个进度条
    total_iterations = n_im_shard * n_cap_shard
    with tqdm(total=total_iterations, desc="Processing shards") as pbar:
        for i in range(n_im_shard):
            ...
            for j in range(n_cap_shard):
                ...
                with torch.no_grad():
                    ...
                    sim = model.forward_sim(opt, im, ca, l)
