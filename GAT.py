...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, querys, keys, values, attention_mask=None, box_relation_embed_matrix = None):
        nodes_q = self.query(querys)
        nodes_k = self.key(keys)
        nodes_v = self.value(values)
        ...
        if box_relation_embed_matrix is not None:
            w_g = box_relation_embed_matrix
            attention_scores = torch.log(torch.clamp(w_g, min=1e-6)) + attention_scores
...
class GATLayer(nn.Module):
    ...
    def forward(self, querys, keys, values, attention_mask=None, position_weight = None):
        attention_output = self.mha(querys, keys, values, attention_mask, position_weight) # multi-head attention
        attention_output = self.fc_in(attention_output)
        attention_output = self.dropout_in(attention_output)
        attention_output = self.bn_in((attention_output + querys).permute(0, 2, 1)).permute(0, 2, 1)
