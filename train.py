...
# from glo_data import create_dataset, create_sampler, create_loader
...
def main():
    ...
    # Load data loaders     data_name, tokenizer, max_seq_len=32, batch_size, workers, opt
...
def train(opt, train_loader, model, epoch, val_loader):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        # Update the model
        model.train_emb(opt, *train_data, device)
...
def adjust_learning_rate(opt, optimizers, epoch):
    ...
    for different parameter groups in the optimizers
    """
    if epoch < 10:
        bert_lr = opt.bert_lr
        other_lr = opt.other_lr
    elif 10 <= epoch < 20:
        bert_lr = opt.bert_lr * 0.1
        other_lr = opt.other_lr * 0.1
    else:
        bert_lr = 0.000001
        other_lr = 0.00001
    optimizers[0].param_groups[0]['lr'] = bert_lr
    optimizers[1].param_groups[0]['lr'] = other_lr
